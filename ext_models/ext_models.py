...
class rGANG(nn.Module):
    def __init__(self, args):
        ...
        layers.append(
            nn.Linear(
                self.args.rgang_fc[-1], self.args.num_hits * self.args.node_feat_size
            )
        )
...
class rGAND(nn.Module):
    def __init__(self, args):
        ...
        for i in range(len(self.args.rgand_sfc) - 1):
            layers.append(
                nn.Conv1d(self.args.rgand_sfc[i], self.args.rgand_sfc[i + 1], 1)
            )
            layers.append(nn.LeakyReLU(negative_slope=args.leaky_relu_alpha))
...
class GraphCNNGANG(nn.Module):
    def __init__(self, args):
        ...
        for i in range(len(self.args.graphcnng_layers) - 1):
            ...
            self.bn_layers.append(
                torch_geometric.nn.BatchNorm(self.args.graphcnng_layers[i + 1])
            )
    ...
    def forward(self, x, labels=None, epoch=None):
        ...
        zeros = torch.zeros(batch_size * self.args.num_hits, dtype=int).to(
            self.args.device
        )
...
class PointNetMixD(nn.Module):
    def __init__(self, args):
        ...
        for i in range(len(self.args.pointnetd_pointfc) - 1):
            layers.append(
                nn.Linear(
                    self.args.pointnetd_pointfc[i], self.args.pointnetd_pointfc[i + 1]
                )
            )
            layers.append(nn.LeakyReLU(negative_slope=args.leaky_relu_alpha))
        ...
        for i in range(len(self.args.pointnetd_fc) - 1):
            layers.append(
                nn.Linear(self.args.pointnetd_fc[i], self.args.pointnetd_fc[i + 1])
            )
    ...
    def forward(self, x, labels=None, epoch=None):
        ...
        if self.args.mask:
            ...
            x[:, :, 2] -= 0.5
        x = self.pointfc(
            x.view(batch_size * self.args.num_hits, self.args.node_feat_size)
        ).view(batch_size, self.args.num_hits, self.args.pointnetd_pointfc[-1])
